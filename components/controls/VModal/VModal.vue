...
const onClose = (e) => {
  if (popup && popup !== e.target && !popup.value.contains(e.target)) {
    if (!clickInside) emit("close");
    else clickInside.value = false;
  }
};
...
.popup {
  ...
  &__body {
    ...
    .popup--image-preview & {
      @media (--desktop-min) {
        padding: calc(var(--gs) * 3.5) calc(var(--gs) * 7) calc(var(--gs) * 3.5)
          calc(var(--gs) * 7);
      }
